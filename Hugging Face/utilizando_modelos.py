...
'''Da mesma forma que fizemos com os datasets, podemos baixar modelos do Hugging Face. 
A forma como estamos fazendo abaixo é a forma mais simples, onde baixamos o modelo e já o utilizamos. Ou seja, o modelo irá para o nosso hd
e será carregado na RAM.
O primeiro parâmetro é a tarefa que queremos realizar, e o segundo parâmetro é o modelo que queremos utilizar.
''' 
modelo = pipeline("summarization", model="facebook/bart-large-cnn", device = "cuda") #Setar o device como cuda é utilizar a GPU.

#O "pipeline" é basicamente a "invocação" do modelo, ou seja, é a forma como vamos utilizar o modelo.
#Esse modelo em específico funciona apenas com textos em inglês, mas temos outros modelos para outras línguas, inclusive português.
...
'''A primeira variável que enviamos para o modelo é sempre o input que queremos processar.
Como estamos trabalhando com sumarização, enviamos um texto, mas se estivéssemos trabalhando com áudio, enviaríamos
as informações de áudio, e assim por diante.'''
...
print(resposta)  # Mostra o resumo gerado pelo modelo